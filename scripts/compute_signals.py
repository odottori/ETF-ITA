        ...
        for symbol in symbols:
            ...
            if signals_data:
                ...
                for signal in signals_data:
                    ...
                    conn.execute("""
                    INSERT INTO signals 
                    (id, date, symbol, signal_state, risk_scalar, explain_code, 
                     sma_200, volatility_20d, spy_guard, regime_filter)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, [
                        next_id, signal['date'], signal['symbol'], signal['signal_state'],
                        signal['risk_scalar'], signal['explain_code'], signal['sma_200'],
                        signal['volatility_20d'], signal['spy_guard'], signal['regime_filter']
                    ])
