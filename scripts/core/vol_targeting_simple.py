...
def calculate_vol_targeting():
    ...
    try:
        ...
        for drawdown_row in drawdown_data:
            symbol, max_dd, days_10, days_20, total = drawdown_row
            
            # Trova volatilità corrente
            current_vol = None
            for vol_row in vol_data:
                if vol_row[0] == symbol:
                    current_vol = vol_row[1]
                    break
            ...
            if current_vol is not None:
                ...
                elif current_vol > BASELINE_VOL_TARGET:
                    vol_adjustment *= 0.9  # 10% riduzione
                    risk_adjustments[symbol] = f"Vol corrente {current_vol:.1%} > {BASELINE_VOL_TARGET:.1%}"
