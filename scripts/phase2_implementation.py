...
Test e backtest con configurazione ottimizzata
"""
    ...
    try:
        ...
        weighted_returns AS (
            SELECT 
                ...
                CASE 
                    ...
                    ELSE daily_return
                END as weighted_return
            FROM portfolio_returns
        ),
        daily_portfolio AS (
            SELECT 
                date,
                AVG(weighted_return) as portfolio_return
        ...
        drawdowns AS (
            SELECT 
                date,
                cumulative_return,
                cumulative_return / MAX(cumulative_return) OVER (ORDER BY date ROWS UNBOUNDED PRECEDING) - 1 as drawdown
            FROM cumulative_returns
        )
        SELECT 
            COUNT(*) as trading_days,
            AVG(portfolio_return) * 252 as annual_return,
            STDDEV(portfolio_return) * SQRT(252) as annual_vol,
            AVG(portfolio_return) * 252 / (STDDEV(portfolio_return) * SQRT(252)) as sharpe,
            MIN(drawdown) as max_drawdown,
            MAX(cumulative_return) - 1 as total_return
        FROM drawdowns
        """
        
        backtest_results = conn.execute(backtest_query).fetchone()
        
        if backtest_results:
            trading_days, annual_return, annual_vol, sharpe, max_dd, total_return = backtest_results
